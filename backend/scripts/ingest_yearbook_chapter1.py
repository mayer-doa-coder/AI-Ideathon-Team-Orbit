"""CLI to (re)build the knowledge base from the curated Yearbook of
Agricultural Statistics 2020 Chapter 1 extract (AEZ, Soil Classification,
Crop Calendar).

This mirrors run_ingestion.py's embed-and-store logic but uses the
document-specific yearbook_aez_soil_crop_calendar_loader instead of the
BARC handbook loader, since the two source PDFs have unrelated internal
structures. Kept as a separate script (rather than parameterizing
ingest_pdf/run_ingestion.py with a loader argument) so the existing BARC
ingestion path is left untouched.

Usage (from /app inside the backend container, or the backend/ venv):
    python -m scripts.ingest_yearbook_chapter1
    python -m scripts.ingest_yearbook_chapter1 --source path/to/other.pdf --title "Other Title"
"""
import argparse
...
from app.rag.loaders.yearbook_aez_soil_crop_calendar_loader import load_and_chunk
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest the Yearbook 2020 Chapter 1 (AEZ/Soil/Crop Calendar) PDF into the kb_chunks knowledge base"
    )

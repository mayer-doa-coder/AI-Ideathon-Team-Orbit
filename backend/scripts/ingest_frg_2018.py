"""CLI to (re)build the knowledge base from the curated Fertilizer
Recommendation Guide-2018 (BARC) OCR extraction.

This mirrors run_ingestion.py's embed-and-store logic but uses the
document-specific fertilizer_guide_loader instead of the BARC handbook
loader, since the two source PDFs have unrelated internal structures.
Kept as a separate script (rather than parameterizing
...
Usage (from /app inside the backend container, or the backend/ venv):
    python -m scripts.ingest_frg_2018
    python -m scripts.ingest_frg_2018 --source path/to/other.pdf --title "Other Title"
"""
import argparse
...
from app.rag.loaders.fertilizer_guide_loader import load_and_chunk
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest the Fertilizer Recommendation Guide-2018 (BARC) PDF into the kb_chunks knowledge base"
    )
